    ...
    return {
      all: () => {
        if (normalized.startsWith('select name, source, path, description, registry_slug, security_status from skills')) {
          return [...this.rows].sort((a, b) => `${a.source}:${a.name}`.localeCompare(`${b.source}:${b.name}`))
        }
        return []
      },
      get: (...args: unknown[]) => {
...
const originalEnv = {
  MC_SKILLS_USER_AGENTS_DIR: process.env.MC_SKILLS_USER_AGENTS_DIR,
  MC_SKILLS_USER_CODEX_DIR: process.env.MC_SKILLS_USER_CODEX_DIR,
  MC_SKILLS_PROJECT_CODEX_DIR: process.env.MC_SKILLS_PROJECT_CODEX_DIR,
  MC_SKILLS_OPENCLAW_DIR: process.env.MC_SKILLS_OPENCLAW_DIR,
  MC_SKILLS_WORKSPACE_DIR: process.env.MC_SKILLS_WORKSPACE_DIR,
...
describe('/api/skills org-agent behavior', () => {
  ...
  beforeEach(() => {
    baseDir = mkdtempSync(join(tmpdir(), 'mc-skills-route-'))
    ...
    const emptyRoot = join(baseDir, 'empty')
    const agentsRoot = join(baseDir, 'agents-root')
    mkdirSync(emptyRoot, { recursive: true })
    mkdirSync(join(agentsRoot, '.skills_library'), { recursive: true })
    process.env.MC_SKILLS_USER_AGENTS_DIR = emptyRoot
    process.env.MC_SKILLS_USER_CODEX_DIR = emptyRoot
    process.env.MC_SKILLS_PROJECT_CODEX_DIR = emptyRoot
    process.env.MC_SKILLS_OPENCLAW_DIR = emptyRoot
    process.env.MC_SKILLS_WORKSPACE_DIR = emptyRoot
    process.env.MISSION_CONTROL_AGENTS_DIR = agentsRoot
    process.env.OPENCLAW_HOME = baseDir
  ...
  it('includes org-agent groups in the grouped response and keeps flat rows source-aware', async () => {
    const sharedPath = join(baseDir, 'shared', 'Retrospective')
    mkdirSync(sharedPath, { recursive: true })
    writeFileSync(join(sharedPath, 'SKILL.md'), '# Retrospective\n\nGlobal workflow.\n', 'utf8')
