...
export async function GET(
  ...
  try {
    ...
    const candidates = getAgentWorkspaceCandidates(
      agentConfig,
      agent.name,
      agent.workspace_path,
    );

    // Handle flat .md file agents (no workspace directory found, but workspace_path is a file)
    const isFlatFile =
      agent.workspace_path &&
      !candidates.length &&
      (() => {
        try {
          return !statSync(agent.workspace_path).isDirectory();
        } catch {
          return false;
        }
      })();
    ...
    if (isFlatFile) {
      let fileContent = "";
      try {
        fileContent = readFileSync(agent.workspace_path, "utf-8");
      } catch {}
      const payload: Record<string, { exists: boolean; content: string }> = {
        "agent.md": { exists: true, content: fileContent },
      };
      for (const file of [
        "identity.md",
        "soul.md",
