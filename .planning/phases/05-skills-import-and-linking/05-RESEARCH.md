...
|----|-------------|------------------|
| SKIL-01 | Org scanner recursively imports SKILL.md files from agent `skills/` subdirectories into the skills catalog | Scanner already iterates agent directories via `syncFilesystemAgentFromPath`; adding a sub-scan of `<agentPath>/skills/` with `org-agent:<name>` source key is a surgical addition. Skills table already has `UNIQUE(source, name)` for safe upserts. |
| SKIL-02 | Agent profile visually links inline skill names to matching SKILL.md catalog entries | `ProfileTab` renders skills as `ProfileChip` spans; matching against `skillsList` from the Zustand store is already loaded by `SkillsPanel`; chips need to become `<button>` elements that open a skill detail drawer or navigate to the skills panel. |
</phase_requirements>
